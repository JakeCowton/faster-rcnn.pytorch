...
class Tester(object):
    ...
    def run(self):
        ...
        self.args.cfg_file = "cfgs/{}_ls.yml".format(self.args.net) if self.args.large_scale else "cfgs/{}.yml".format(self.args.net)
        ...
        if self.args.net == 'vgg16':
            fasterRCNN = vgg16(imdb.classes, pretrained=False, class_agnostic=self.args.class_agnostic)
        elif self.args.net == 'res101':
            fasterRCNN = resnet(imdb.classes, 101, pretrained=False, class_agnostic=self.args.class_agnostic)
        elif self.args.net == 'res50':
            fasterRCNN = resnet(imdb.classes, 50, pretrained=False, class_agnostic=self.args.class_agnostic)
